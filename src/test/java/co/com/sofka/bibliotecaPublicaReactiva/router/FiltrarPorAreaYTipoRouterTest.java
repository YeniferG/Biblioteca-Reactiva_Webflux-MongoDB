...
@ContextConfiguration(classes= {FiltrarPorAreaYTipoRouter.class, UseCaseBuscarPorAreaYTipo.class, RecursoMapper.class})
class FiltrarPorAreaYTipoRouterTest {
    ...
    @Test
    public void filtrarPorAreaYTipoTest(){
        ...
        when(repositorio.findByAreaTematicaAndTipo(recurso1.getAreaTematica().toString(),
                recurso1.getTipo().toString()))
                .thenReturn(Flux.just(recurso1, recurso2));

        webTestClient.get()
                .uri("/recursos/filtrar/FISICA/DOCUMENTAL")
                .exchange()
                .expectStatus().isOk()
        ...
        Mockito.verify(repositorio,Mockito.times(1)).findByAreaTematicaAndTipo("FISICA", "DOCUMENTAL");
    }
